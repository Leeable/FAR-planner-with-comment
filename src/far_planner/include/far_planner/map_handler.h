...
struct MapHandlerParams {
    ...
    float sensor_range;
    float floor_height;
    float cell_length;  //单个cell的长度
    float cell_height;  //单个cell的高度
...
public:
    bool dilate_factor = false;
    bool dilate_world_obs = false;
    MapHandler() = default;
    ~MapHandler() = default;
...
private:
    ...
    inline void AssignFlatTerrainCloud(const PointCloudPtr& terrainRef, PointCloudPtr& terrainFlatOut) {    //把点云从三维压缩到二维，z值被放到了intensity里
    ...
    std::vector<int> util_obs_modified_list_;           //用来存对应的ind是不是obs      1的话就是obs
    std::vector<int> util_free_modified_list_;          //用来存对应的ind是不是free     1的话就是free
