...
typedef std::unordered_set<Eigen::Vector3i,eigen_hash,eigen_comp> SubSet;
...
public:
    ...
    // kdTree cloud
    static PointKdTreePtr kdtree_new_cloud_;    //存储新的点云
    static PointKdTreePtr kdtree_filter_cloud_;
    // contour
    static std::unordered_set<cv::Point2f,point_hash , point_comp> contour_;
    /*
    ...
    */
    ...
    static void ConvertCTNodeStackToPCL(const CTNodeStack& ctnode_stack, 
                                        const PointCloudPtr& point_cloud_ptr);
    //new add
    static void ConvertNavNodePtrStackToPCL(const NodePtrStack &navnode_stack,
									  const PointCloudPtr &point_cloud_ptr);
    ...
    //geometric constraint function 用来约束（检测）cur_p 到线段（from_p, end_p）在空间中的距离
    ...
    template <typename Point>
    static bool PointInsideAPoly(const std::vector<Point>& poly, const Point& p) {  //默认c为0即false不相交
