...
public:
    ...
    /**
     * @brief outrange potential node match with ctnode
     * 
     * @param out_node_ptr 
     * @param near_nodes 
     * @return NavNodePtr 
    ...
    static inline void MatchCTNodeWithNavNode(const CTNodePtr& ctnode_ptr, const NavNodePtr& node_ptr) {
        ...
        //ctnode的is_global_match设为true 因为它和push到global_graph的nav_node匹配到
        ctnode_ptr->is_global_match = true;
        //ctnode的nav_node_id对应的就是node_ptr的id
        ctnode_ptr->nav_node_id = node_ptr->id;
        //把该nav_node的ctnode属性用匹配到的ctnode填充
        node_ptr->ctnode = ctnode_ptr;
        //ctnode的is_contour_match设为true
...
private:
     ...
     * 判断polygon图形每个matched的点是否按逆时针封闭，如果有几个点没被matched，那么判断这几个点是不是构成polygon的必要点
     */
