...
bool is_free_nav_goal_;             // the attemptable navigation status (True)->Non-attempts
NodePtrStack potential_node;
bool nav_flag = false;
int mis_counter = 0;
...
*/
inline void UpdateVGraph(const NodePtrStack& vgraph) {current_graph_ = vgraph;};

/**
 * Update Global Graph Traversability Status and gscores through connect_nodes
...
/**
 * @brief Generate path to goal based on traversibility result 
 * 如果终点是被发现的，那么导航点返回global_path的下一步，
 * 如果终点未被发现，那么导航点直接返回终点，并开启尝试模式
