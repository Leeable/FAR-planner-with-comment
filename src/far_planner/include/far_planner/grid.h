...
public:
                ...
                const Eigen::Vector3d& resolution = Eigen::Vector3d(1, 1, 1), int dimension = 3)
  {
    ...
    {
      cells_[i] = init_value;   //    把每个terrain_node_ptr给到cell里 初始是node都是null
      subs_[i] = ind2sub_(i);   //    
    }
  }
  ...
  bool InRange(const Eigen::Vector3i& sub) const
  {
    ...
    for (int i = 0; i < dimension_; i++)
    {
      in_range &= sub(i) >= 0 && sub(i) < size_(i);   //按位与赋值运算符（＆=）表示两个操作数的二进制，对它们进行按位AND运算并将结果分配给变量。
    }
    return in_range;
  }
...
private:
  Eigen::Vector3d origin_;                //  中心点位置
  Eigen::Vector3i size_;                  //  grid的尺寸
  Eigen::Vector3d resolution_;            //  根据voxel得到的分辨率
  Eigen::Vector3d resolution_inv_;        //  分辨率的倒数
  std::vector<_T> cells_;                 //  grid里的每一个点
  std::vector<Eigen::Vector3i> subs_;     //  是grid里的点，从（0，0，0）一直到（size.x,size.x,0）
  int cell_number_;                       //  cell的个数
  int dimension_;                         //  grid的维度
