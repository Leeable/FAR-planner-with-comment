...
private:
    ...
    ros::Publisher  laser_timer_pub_;
    ros::Subscriber record_data_;
    ros::Subscriber ouputgrid_;
    ros::Timer planning_event_;
    
    std_msgs::Float32 runtimer_, plan_timer_, laser_timer_;
    ...
    GlobalGridParams    output_grid_params_;
    

    // 判断是否记录waypoint、odom、goal、connection
    bool is_record;
    // 判断是否输出grid
    bool is_output;
    ...
    //  自己写的输出data
    inline void RecordDataCallBack(const std_msgs::BoolConstPtr& msg){
        is_record =  msg->data;
    }
    //  自己写的输出grid
    inline void OutputGridCallBack(const std_msgs::BoolConstPtr& msg){
        is_output = msg->data;
    }
