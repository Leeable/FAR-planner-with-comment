...
// 动态障碍物的grid 30m x 30m 
void ScanHandler::Init(const ScanHandlerParams& params) {
    scan_params_ = params;
    row_num_ = std::ceil(scan_params_.terrain_range * 2.0f / scan_params_.voxel_size);  //200
    if (row_num_ % 2 == 0) row_num_ ++;
    col_num_ = row_num_;            //201 x 201
    level_num_ = std::ceil(scan_params_.ceil_height * 2.0f / scan_params_.voxel_size);  //27
    if (level_num_ % 2 == 0) level_num_ ++;
    Eigen::Vector3i grid_size(row_num_, col_num_, level_num_);
    Eigen::Vector3d grid_origin(0,0,0);
    Eigen::Vector3d grid_resolution(scan_params_.voxel_size, scan_params_.voxel_size, scan_params_.voxel_size);     //  0.15 * 0.15 * 0.15
...
void ScanHandler::UpdateRobotPosition(const Point3D& robot_pos) {
    ...
    grid_origin.x() = robot_pos.x - (scan_params_.voxel_size * row_num_) / 2.0f;        // voxel_size * row_num 相当于0.15*201 = 30m的距离，除以2就是15，相当于从odom的坐标，减去15，作为原点
    ...
    center_sub_ = voxel_grids_->Pos2Sub(Eigen::Vector3d(robot_pos.x, robot_pos.y, robot_pos.z));        //在dynamic_grid的动态图上的中心点的sub
...
void ScanHandler::SetCurrentScanCloud(const PointCloudPtr& scanCloudIn, const PointCloudPtr& freeCloudIn) {
    ...
    FARUtil::RemoveOverlapCloud(copyObsScanCloud, freeCloudIn, true);       //从刚扫描出来的copyObsScanCloud中分离free_cloud
    ...
    for (const auto& point : copyObsScanCloud->points) { // assign obstacle scan voxels
        const float r = pcl::euclideanDistance(point, center_p_);       // 点到小车的距离
        ...
        Eigen::Vector3i c_sub = voxel_grids_->Pos2Sub(Eigen::Vector3d(point.x, point.y, point.z));  //把点转换到obstacle grid下的sub
        if(temp_sub == c_sub){
            continue;
        }
        temp_sub = c_sub;
