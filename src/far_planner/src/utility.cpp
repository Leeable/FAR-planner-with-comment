...
void FARUtil::RemoveNanInfPoints(const PointCloudPtr &cloudInOut)
{
	...
	for (const auto &p : cloudInOut->points)
	{
		...
		{
			if (FARUtil::IsDebug)
				ROS_WARN_ONCE("FARUtil: nan or inf point detected.");
			continue;
		}
									  ...
									  const PointCloudPtr &cloudNew)
{
	...
	*temp_new_cloud = *cloudIn + *cloudRefer;
	FARUtil::FilterCloud(temp_new_cloud, FARUtil::kLeafSize * 2.0);	//再进一步过滤obs_cloud
...
void FARUtil::CropCloudWithinHeight(const PointCloudPtr &cloudInOut, const float &height)
{
	...
	for (const auto &p : cloudInOut->points)
	{
		if (!FARUtil::IsPointInToleratedHeight(p, height))
			continue;
		temp_cloud.points[idx] = p;
		idx++;
	}
...
bool FARUtil::IsSameFrameID(const std::string &cur_frame, const std::string &ref_frame)
{
	...
	if (cur_frame[0] == '/')
		str1 = cur_frame.substr(1);
	if (ref_frame[0] == '/')
		str2 = ref_frame.substr(1);
	return str1 == str2;
}
...
{
	//如果环境变化了，就会产生新的obs_nodes 那么nav_node附近就可能多出来许多points
	const std::size_t near_c = FARUtil::PointInNewCounter(p, FARUtil::kMatchDist);
	// FARUtil::KNewPointC = 5
									 ...
									 const PointKdTreePtr &KdTree)
{
	...
	// std::isfinite 判断浮点数的范围正不正确，正确返回1
	if (!std::isfinite(pcl_p.x) || !std::isfinite(pcl_p.y) || !std::isfinite(pcl_p.z))
	{
		return 0;
	}
	//在kdTree中以pcl_p为中心radius的范围内寻找点，点的index放在pointSearchInd，点离pcl_p的距离放在pointSearchSqDis
...
{
	// FARUtil::kdtree_new_cloud_ 存的是新的obs点云，如果环境没有变化，那么就没有新的obs
...
float FARUtil::MarginAngleNoise(const float &dist, const float &max_shift_dist, const float &angle_noise)
{
	...
	//当max_shift_dist = 0.8  angle_noise = 15°时，dist=3.09时它和max_shift_dist相等，分成三个层级
	// dist:0~0.8时：margin_angle_noise = 90°
	// dist:0.8~3.09时：90°~15°之间
	// dist：大于3.09时 margin_angle_noise = 15°
...
bool FARUtil::IsInCoverageDirPairs(const Point3D &diff_p, const NavNodePtr &node_ptr)
{
	...
	//margin_angle_noise给出一个30°
	//diff_p.norm_flat()是大于1.75的一个值，FARUtil::kNearDist为0.8
	/*
		默认FARUtil::kAngleNoise * 2.0f 为30°，则dist * sin(margin_angle_noise) >= 1.75*0.5 = 0.875
		0.875大于kNearDist的，所以返回30°
	*/
	const float margin_angle_noise = FARUtil::MarginAngleNoise(diff_p.norm_flat(),
															   FARUtil::kNearDist,
...
float FARUtil::NoiseCosValue(const float &dot_value, const bool &is_large, const float &noise)
{
	...
	margin_theta = std::min(std::max(margin_theta, 0.0), M_PI); //不能超过180° 即3.14
...
{
	if (cloudRef->empty() || cloudInOut->empty())
		return;
	PointCloudPtr temp_cloud(new pcl::PointCloud<PCLPoint>());
	...
	// 对于同一个point，已经经过分类的是free，刚扫描出来的都认为是obs，那么就先经过obs把值拉到255，再经过free把是free的point拉到0，这样就分离了obs和free
	FARUtil::ResetCloudIntensity(cloudInOut, true);		// obs  intensity 225
	FARUtil::ResetCloudIntensity(ref_cloud, false);		// free intensity 0    会把obs的point的覆盖掉
	*temp_cloud = *cloudInOut + *ref_cloud;				// temp_cloud = obs + free
	const float leaf_size = FARUtil::kLeafSize * 1.2;
	FARUtil::FilterCloud(temp_cloud, leaf_size);		// 过滤
	cloudInOut->clear(), cloudInOut->resize(temp_cloud->size());
	std::size_t idx = 0;
	for (const auto &p : temp_cloud->points)
	{
		if (p.intensity < 255.0)
			continue;
								 ...
								 const float &radius)
{
	...
	if (H < FARUtil::kEpsilon)
	{ // co-linear
		//共线就返回由center指向end的归一化向量（一般情况）
...
//纠正ctnode的surf_dir的方向 使其与nav_node的surf_dir一致
void FARUtil::CorrectDirectOrder(const PointPair &ref_dir, PointPair &dirInOUt)
{
	PointPair temp_dir = dirInOUt;
	// cos值 如果正确的话，匹配到的nav_node的surf_dir和ctnode的surf_dir是一致的，即两者first的角度应该重合，cos值为1
	//正常情况下，左边应该是2  右边应该是一个-2到2的值。
...
PointPair FARUtil::RANSACSurfDirs(const std::deque<PointPair> &surf_dirs_stack, const float &margin, std::size_t &inlier_size)
{
	...
	if (pillar_size > inlier_size)
	{ // this node should be a pillar
...
//new add
void FARUtil::ConvertNavNodePtrStackToPCL(const NodePtrStack &navnode_stack,
									  const PointCloudPtr &point_cloud_ptr)
{
	const std::size_t N = navnode_stack.size();
	point_cloud_ptr->clear(), point_cloud_ptr->resize(N);
	for (std::size_t i = 0; i < N; i++)
	{
		point_cloud_ptr->points[i] = FARUtil::Point3DToPCLPoint(navnode_stack[i]->position);
	}
}
...
bool FARUtil::IsInCylinder(const Point3D &from_p, const Point3D &end_p, const Point3D &cur_p, const float &radius, const bool &is_2D)
{
	...
	*/
	...
	//  在A*里f(n)=g(n)+h(n) ，对于两个node m和n  存在 d(m, n) + h'(n) > h'(m)
	//  也即temp_line_dist + radius > proj_scalar  而这个radius即作者的Cylinder半径
	...
	//此处的temp_line_dist为cur_p到线段(from_p，end_p)的距离
...
bool FARUtil::IsVoteTrue(const std::deque<int> &votes, const bool &is_balance)
{
	...
	//只要sum的值大于N/factor   即 N/2或N/3  就返回true
...
//由于ordered_votes从大到小排列，如果得分低，说明可能是fake_vote  idx越大，越说明是fake_vote
int FARUtil::VoteRankInVotes(const int &c, const std::vector<int> &ordered_votes)
{
	//遍历ordered_votes
	int idx = 0;
	//如果分数c小于ordered_votes[idx](从大到小排列的)，则idx++
