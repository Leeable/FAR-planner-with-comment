...
void TerrainPlanner::Init(const ros::NodeHandle& nh, const TerrainPlannerParams& params) {
    nh_ = nh;
    tp_params_ = params;
    row_num_ = std::ceil((FARUtil::kLocalPlanRange + tp_params_.radius) * 2.0f / tp_params_.voxel_size);    // 88
    col_num_ = row_num_;
    TerrainNodePtr init_terrain_node_ptr = NULL;
    Eigen::Vector3i grid_size(row_num_, col_num_, 1);   //88 * 88 *1
    Eigen::Vector3d grid_origin(0,0,0);
    Eigen::Vector3d grid_resolution(tp_params_.voxel_size, tp_params_.voxel_size, tp_params_.voxel_size);       //  0.15 * 0.15 * 0.15
    terrain_grids_ = std::make_unique<grid_ns::Grid<TerrainNodePtr>>(grid_size, init_terrain_node_ptr, grid_origin, grid_resolution, 3);    //cells里存放的是node
...
void TerrainPlanner::UpdateCenterNode(const NavNodePtr& node_ptr) {
    ...
    is_grids_init_ = true;
    this->ResetGridsPositions();    //重新更新grid范围内的点
    this->ResetGridsOccupancy();    //重置terrain_grids_->GetCell(i)->is_occupied = false
}

void TerrainPlanner::SetLocalTerrainObsCloud(const PointCloudPtr& obsCloudIn) {
    //is_grids_init_默认是false
    ...
    this->GridVisualCloud();            //只有dynamic env才会有
}
