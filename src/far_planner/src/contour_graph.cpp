...
void ContourGraph::Init(const ContourGraphParams& params) {
    ...
    ALIGN_ANGLE_COS = cos(M_PI - FARUtil::kAcceptAlign / 2.0f);     //-0.99939
...
void ContourGraph::UpdateContourGraph(const NavNodePtr& odom_node_ptr,
    ...
    //从filtered_contours里取出每个polygon，一个poly是一组point，代表一个polygon
    for (const auto& poly : filtered_contours) {
        PolygonPtr new_poly_ptr = NULL;
        //把poly单纯的一组点转换成一个polygon的属性
        this->CreatePolygon(poly, new_poly_ptr);
        //! 把这个polygon加入到contour_polygons_里
    ...
    for (const auto& poly_ptr : ContourGraph::contour_polygons_) {
        ...
        if (poly_ptr->is_pillar) {
            ...
            ctnode_stack.clear();
            //new add  处理相互连接之间都小于0.8的点
            int K = poly_ptr->vertices.size();
            std::unordered_set<int> removed_idx;
            while(K>20){
                removed_idx.clear();
                for (int idx=0; idx<K; idx++) {
                    int front_idx = FARUtil::Mod(idx-1, K);
                    int back_idx = FARUtil::Mod(idx+1, K);
                    const auto p = poly_ptr->vertices[idx];
                    const auto p1 = poly_ptr->vertices[front_idx];
                    const auto p2 = poly_ptr->vertices[back_idx];
                    const auto dist1 = (p-p1).norm();
                    const auto dist2 = (p-p2).norm();
                    if((dist1<1.5 && dist2<0.95)||(dist1<0.95 && dist2<1.5)){
                        removed_idx.insert(idx);
                    }
                }
                if (!removed_idx.empty()) { // clear contour with vertices size less that 3
                    std::vector<Point3D> temp_vertices = poly_ptr->vertices;
                    poly_ptr->vertices.clear();
                    for (int i=0; i<temp_vertices.size(); i++) {
                        ...
                        poly_ptr->vertices.push_back(temp_vertices[i]);
                        }
                }
            ...
            for (int idx=0; idx<N; idx++) {
                ...
                this->AddCTNodeToGraph(ctnode_stack[idx]);      // contour_graph_
            }
...
void ContourGraph::MatchContourWithNavGraph(const NodePtrStack& global_nodes, const NodePtrStack& near_nodes, CTNodeStack& new_convex_vertices) {
    /*
        分析以下第一次，global_nodes为空 near_nodes只有odom本身此时matched_node = NULL，
        contour_graph_里的ctnode的is_global_match都为false，得到new_convex_vertices
    */
    ...
    for (const auto& ctnode_ptr : ContourGraph::contour_graph_) { // Get new vertices
        if (!ctnode_ptr->is_global_match && ctnode_ptr->free_direct != NodeFreeDirect::UNKNOW) {
            if (ctnode_ptr->free_direct != NodeFreeDirect::PILLAR) { // check wall contour
                const float dot_value = ctnode_ptr->surf_dirs.first * ctnode_ptr->surf_dirs.second; //计算顶点两个方向向量的cos值，如果小于-0.99 夹角180度
...
bool ContourGraph::IsCTNodesConnectFromContour(const CTNodePtr& ctnode1, const CTNodePtr& ctnode2) {
    //主要判断ctnode1和ctnode2是不是在同一个polygon里，不在同一个polygon里 或 ctnode1==ctnode2 返回false
...
NavNodePtr ContourGraph::MatchOutrangeNodeWithCTNode(const NavNodePtr& out_node_ptr, const NodePtrStack& near_nodes) {
    ...
    for (const auto& node_ptr : near_nodes) {
        ...
        if (IsContourLineMatch(node_ptr, out_node_ptr, matched_ctnode)) {
            //判断outnode和（near_node,matched_ctnode)的距离
...
bool ContourGraph::IsContourLineMatch(const NavNodePtr& inNode_ptr, const NavNodePtr& outNode_ptr, CTNodePtr& matched_ctnode) {
    //取出innode_ptr(nav_node 【near_nodes】)匹配到的ctnode
    ...
    //若next_ctnode无global_match 且 next_ctnode不等于ctnode_ptr 且 IsInCylinder为true prev_ctnode和线段（ctnode_ptr，next_ctnode）的空间距离约束
    ...
    {
        //若next_ctnode的位置不在MarginRange内，且ctnode_ptr和next_ctnode的距离大于kMatchDist
            ...
            (ctnode_ptr->position - next_ctnode->position).norm_flat() > FARUtil::kMatchDist) 
        {
            ...
            //计算line1 （innode -> outnode）和line2 (ctnode -> next_ctnode)的相似程度
...
void ContourGraph::EnclosePolygonsCheck() {
    //polys_ctnodes_存放每一个polygon的首个ctnode 通过front或back可以循环到该polygon的每一个ctnodes  只有不是pillar类型的才有这个
    for (const auto& ctnode_ptr : ContourGraph::polys_ctnodes_) { // loop each polygon
        ...
        while (cur_ctnode_ptr != start_ctnode_ptr) {
            if (!cur_ctnode_ptr->is_global_match) {         //如果该点没有match到，往front方向继续找，并跳过下面的步骤
                cur_ctnode_ptr = cur_ctnode_ptr->front;
                continue;
            }
            //用来判断那些没有globalmatch到的点是不是组成polygon必须要的点
            CTNodePtr block_vertex = NULL;
            if (!IsCTNodesConnectWithinOrder(pre_ctnode_ptr, cur_ctnode_ptr, block_vertex) && block_vertex != NULL) {
                //创建ctnode的时候，ctnode_ptr->is_ground_associate = false;
                if (block_vertex->is_ground_associate && FARUtil::IsPointInMarginRange(block_vertex->position)) {
                    //如果满足is_ground_associate为true且该点在margin_range里，那么这个点就是构成contour的必要点
...
//从near_nodes中找到离contour_graph_的这个ctnode最近的一个near_node
...
void ContourGraph::AnalysisConvexityOfCTNode(const CTNodePtr& ctnode_ptr) {
    if (ctnode_ptr->surf_dirs.first  == Point3D(0,0,-1) || ctnode_ptr->surf_dirs.second == Point3D(0,0,-1) || ctnode_ptr->poly_ptr->is_pillar) {
        ...
        ctnode_ptr->poly_ptr->is_pillar = true;             //这个是由N个ctnode组成的polygon  整个polygon是pillar
        ctnode_ptr->free_direct = NodeFreeDirect::PILLAR;
        return;
    }
    bool is_wall = false;
