...
void MapHandler::Init(const MapHandlerParams& params) {
    map_params_ = params;
    const int row_num = std::ceil(map_params_.grid_max_length / map_params_.cell_length);       //  200
    const int col_num = row_num;
    int level_num = std::ceil(map_params_.grid_max_height / map_params_.cell_height);           //  125
    neighbor_Lnum_ = std::ceil(map_params_.sensor_range * 2.0f / map_params_.cell_length) + 1;  //  13
    neighbor_Hnum_ = 5; 
    ...
    // inlitialize grid 
    Eigen::Vector3i pointcloud_grid_size(row_num, col_num, level_num);          //  200 * 200 * 125
    Eigen::Vector3d pointcloud_grid_origin(0,0,0);
    Eigen::Vector3d pointcloud_grid_resolution(map_params_.cell_length, map_params_.cell_length, map_params_.cell_height);          //  5 * 5 * 0.8
    ...
    Eigen::Vector3i height_grid_size(height_dim, height_dim, 1);    //  89 * 89 * 1
    Eigen::Vector3d height_grid_origin(0,0,0);
    Eigen::Vector3d height_grid_resolution(FARUtil::robot_dim, FARUtil::robot_dim, FARUtil::kLeafSize);     //0.8 * 0.8 * 0.15
...
void MapHandler::SetMapOrigin(const Point3D& ori_robot_pos) {
    Point3D map_origin;
    const Eigen::Vector3i dim = world_obs_cloud_grid_->GetSize();   //把小车设置在中心位置   
    ...
    map_origin.z = ori_robot_pos.z - (map_params_.cell_height * dim.z()) / 2.0f - FARUtil::vehicle_height; // From Ground Level     (-500,-500,-50.25)
...
void MapHandler::UpdateRobotPosition(const Point3D& odom_pos) {
    ...
    robot_cell_sub_ = world_obs_cloud_grid_->Pos2Sub(Eigen::Vector3d(odom_pos.x, odom_pos.y, odom_pos.z));  //机器人在grid地图上的位置
    // Get neighbor indices
    neighbor_free_indices_.clear(), neighbor_obs_indices_.clear();
    const int N = neighbor_Lnum_ / 2;   //6
    const int H = neighbor_Hnum_ / 2;   //2
    Eigen::Vector3i neighbor_sub;
    for (int i = -N; i <= N; i++) {     //以机器人为中心建立一个13x13的neighbor grid
        ...
        for (int j = -N; j <= N; j++) {
            ...
            if (world_obs_cloud_grid_->InRange(neighbor_sub)) { //如果这个sub在范围内，则将sub转换成indice，存到neighbor_free里
                int ind = world_obs_cloud_grid_->Sub2Ind(neighbor_sub);
                neighbor_free_indices_.insert(ind);         //这个层面的z是固定的
            }
            for (int k =-H; k <= H; k++) {
                neighbor_sub.z() = robot_cell_sub_.z() + k;
                if (world_obs_cloud_grid_->InRange(neighbor_sub)) {
                    int ind = world_obs_cloud_grid_->Sub2Ind(neighbor_sub);
                    neighbor_obs_indices_.insert(ind), neighbor_free_indices_.insert(ind);  //正常分别是845个和1014个
                }
            }
        }
    }
...
void MapHandler::SetTerrainHeightGridOrigin(const Point3D& robot_pos) {
    ...
    const Eigen::Vector3d res = terrain_height_grid_->GetResolution();  //（0.8，0.8，0.15）
    const Eigen::Vector3i dim = terrain_height_grid_->GetSize();        //（89，89，1）
...
void MapHandler::GetSurroundFreeCloud(const PointCloudPtr& freeCloudOut) {
    ...
    for (const auto& neighbor_ind : neighbor_free_indices_) {
        if (world_free_cloud_grid_->GetCell(neighbor_ind)->empty()) continue;   //neighbour只存ind，要判断有没有还要根据ind去cell[ind]里找
        *freeCloudOut += *(world_free_cloud_grid_->GetCell(neighbor_ind));      //得到neighbour范围内的free_point_cloud
    }
}

void MapHandler::UpdateObsCloudGrid(const PointCloudPtr& obsCloudInOut) {
    // 修改 加入了空洞卷积结构
    dilate_world_obs=!dilate_world_obs;
    ...
    for (const auto& point : obsCloudInOut->points) {
        Eigen::Vector3i sub = world_obs_cloud_grid_->Pos2Sub(Eigen::Vector3d(point.x, point.y, point.z));   //一个sub能对应很多个cloudpoint，也就是一个ind对应了很多个cloud_points
        
        int row = sub.x() % 2;
        if(dilate_world_obs == 0 ){
            if(row == 0 ) continue;
        }
        if(dilate_world_obs == 1 ){
            if(row == 1 ) continue;
        }

        if (!world_obs_cloud_grid_->InRange(sub)) continue;
        const int ind = world_obs_cloud_grid_->Sub2Ind(sub);
        if (neighbor_obs_indices_.find(ind) != neighbor_obs_indices_.end()) {   //如果在neighbour范围找到了该点
            world_obs_cloud_grid_->GetCell(ind)->points.push_back(point);       //就把该点push进对应的cell[ind]  这样就可以通过唯一的sub去找到该点
            obs_valid_ptr->points.push_back(point);
            util_obs_modified_list_[ind] = 1;
            global_visited_induces_[ind] = 1;
        }
    }
    *obsCloudInOut = *obs_valid_ptr;        //相当于用world_obs_cloud->InRange筛了一遍
    // Filter Modified Ceils    //对obs的cloud_point进行稀疏化
    for (int i = 0; i < world_obs_cloud_grid_->GetCellNumber(); ++i) {
      if (util_obs_modified_list_[i] == 1) FARUtil::FilterCloud(world_obs_cloud_grid_->GetCell(i), FARUtil::kLeafSize); //如果为1，那么就把里面的cloud_points进行一个过滤，这样cell[ind]里面的点就会少一些
    }
}
...
void MapHandler::UpdateFreeCloudGrid(const PointCloudPtr& freeCloudIn){
    ...
    for (int i = 0; i < world_free_cloud_grid_->GetCellNumber(); ++i) {
      if (util_free_modified_list_[i] == 1) FARUtil::FilterCloud(world_free_cloud_grid_->GetCell(i), FARUtil::kLeafSize);   //和obs一样的
    }
}
...
void MapHandler::AdjustCTNodeHeight(const CTNodeStack& ctnodes) {
    ...
    for (auto& ctnode_ptr : ctnodes) {
        float min_th, max_th;
        //计算完NearestHeightOfRadius以后，is_ground_associate为true
...
//按高度z来区分的话，根据intensity来进行可视化，就会发现有墙的地方z值高，呈现绿色，没有墙的地方z值低，呈现红色 use_rainbow
void MapHandler::UpdateTerrainHeightGrid(const PointCloudPtr& freeCloudIn, const PointCloudPtr& terrainHeightOut) { //输入的是free_surround_cloud 所以只要有的都是可以通行的
    ...
    for (const auto& point : copy_free_ptr->points) {
        ...
        for (const auto& sub : subs) {
            ...
            const int ind = terrain_height_grid_->Sub2Ind(sub);
            if (terrain_grid_occupy_list_[ind] == 0) {          //如果没有遍历过这个点（ps.因为list都填0了，所以第一次遇到一个新点，就是0，因为膨胀的关系，很多点可能会重复出现）
                terrain_height_grid_->GetCell(ind).resize(1);
                terrain_height_grid_->GetCell(ind)[0] = point.z;    //存point的z值
            } else {
                terrain_height_grid_->GetCell(ind).push_back(point.z);
            }
            terrain_grid_occupy_list_[ind] = 1;             //0代表没有遍历过该点，1代表遍历过该点     在遍历过的点里，ind对应的是free_point的ind
        }
    }
    const int N = terrain_grid_occupy_list_.size();
    this->TraversableAnalysis(terrainHeightOut);                //更新得到可以通行的terrainHeightOut
...
void MapHandler::TraversableAnalysis(const PointCloudPtr& terrainHeightOut) {
    ...
    const float H_THRED = map_params_.height_voxel_dim;           //  0.3
    std::fill(terrain_grid_traverse_list_.begin(), terrain_grid_traverse_list_.end(), 0);
    // Lambda Function
    auto IsTraversableNeighbor = [&] (const int& cur_id, const int& ref_id) {
        if (terrain_grid_occupy_list_[ref_id] == 0) return false;           //如果该点在terrain_grid下没有探索过，那么就是false ，  如果探索过，就更新一下该id下的平均高度
        ...
        if (counter > 0) {                                                  //count=0代表探索过该点，但是
    ...
    while (!q.empty()) {
        ...
        q.pop_front();
        if (terrain_grid_occupy_list_[cur_id] != 0) {   //该id在occupy_list是否访问过
            if (!is_robot_terrain_init) {   //如果terrain_init为false
                float avg_h = 0.0f;
                int counter = 0;
                ...
                if (counter > 0) {              //求平均高度
                    avg_h /= (float)counter;
                    terrain_height_grid_->GetCell(cur_id).resize(1);        //  把cell[cur_id]里的points改成一个
                    terrain_height_grid_->GetCell(cur_id)[0] = avg_h;       //  存z的平均值
                    AddTraversePoint(cur_id);                               //  因为cur_id对应的是free_point的id，所以是可以通行的点
        ...
        for (int i=0; i<4; i++) {
            ...
            if (!visited_set.count(ref_id) && (!is_robot_terrain_init || IsTraversableNeighbor(cur_id, ref_id))) {  // 对ref_id有要求的，必须是可通行的neighbour
                q.push_back(ref_id);
...
void MapHandler::GetNeighborCeilsCenters(PointStack& neighbor_centers) {    //这个是局部的
    if (!is_init_) return;  
    neighbor_centers.clear();
    for (const auto& ind : neighbor_obs_indices_) { //它从neighbor_obs_indices_里获取的，每次这个neighbor_obs都会clear再更新周围的点
...
void MapHandler::GetOccupancyCeilsCenters(PointStack& occupancy_centers) {  //这个是全局的
    if (!is_init_) return;
    occupancy_centers.clear();
    const int N = world_obs_cloud_grid_->GetCellNumber();
    for (int ind=0; ind<N; ind++) {
        if (global_visited_induces_[ind] == 0) continue;        //把从global_visited_induces_里瞧过的点才拿出来
        Point3D center_p(world_obs_cloud_grid_->Ind2Pos(ind));  //这个瞧过的点从ind转成pos
