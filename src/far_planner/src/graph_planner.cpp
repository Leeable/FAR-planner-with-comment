...
// free_terain_grid允许goal为中心，以偏移的origin为原点，x,y离原点距离其最大4.05范围内的所有点
void GraphPlanner::Init(const ros::NodeHandle& nh, const GraphPlannerParams& params) {
    ...
    Eigen::Vector3i grid_size(col_num, col_num, 1);         //  27 * 27 * 1
...
void GraphPlanner::UpdateGoalNavNodeConnects(const NavNodePtr& goal_ptr)
{
    ...
    for (const auto& node_ptr : current_graph_) {
        ...
        if (it != goal_ptr->edge_votes.end() && FARUtil::IsVoteTrue(it->second, false)) {
            ...
            node_ptr->is_block_to_goal = true;          //  如果current_graph里的nav_node无法连接到goal，这些nav_node就是block_to_goal
        }
    }
}
                              ...
                              bool& _is_free_nav) 
{
    ...
    //check free navigation command         允许尝试的话，默认command_is_free_nav_为false
    ...
    // 如果我yaml开启了自动尝试，但是在窗口中关闭了自动尝试，那么他就会根据goal周围的情况来判断需不需要开启自动尝试
    if (gp_params_.is_autoswitch && command_is_free_nav_) {
        if (goal_ptr->is_free_traversable || (is_free_nav_goal_ && is_global_path_init_ && path_momentum_counter_ < gp_params_.momentum_thred)) {
            is_free_nav_goal_ = true;   //如果是可通行区域（最主要的），就不需要尝试了
        } else {
            is_free_nav_goal_ = false;  //否则就需要进行尝试
        }
    }   
    _is_free_nav = is_free_nav_goal_;
    // 追溯goal的父节点
    const NavNodePtr reach_nav_node = is_free_nav_goal_ ? goal_ptr->free_parent : goal_ptr->parent;     //如果goal环境是已知的，就从goal开始找属性is_covered为true的nav_node，否则为普通的nav_node
    //如果reach_nav_node找到了（一般来说就是goal所在的区域已经探索过，就能从goal返回一个父节点）
    if (reach_nav_node != NULL) { // valid path found
        //如果 path_momentum_counter不满足，则会重新规划路径，另外它每执行momentum_thred次之后，也会重新规划一次
        ...
        {   // check for momentum path
            ...
            if (cur_waypoint_dist > gp_params_.adjust_radius) {
                if ((odom_node_ptr_->position - last_planning_odom_).norm() < gp_params_.momentum_dist) { // movement momentum
                    ...
                    _nav_node_ptr = this->NextNavWaypointFromPath(global_path, goal_ptr);   // 就还没到达该导航点的时候，不断发送该导航点
        ...
        // 第一步都会走这个，创建一个global_path
        NodePtrStack cur_path;
        if (this->ReconstructPath(goal_ptr, is_free_nav_goal_, cur_path)) { //找到和终点依次连起来的nav_node
            ...
            this->RecordPathInfo(global_path);  //这里将is_global_path_init_设置为true ， 并且将global_path保存到record_path中
            return true;
        }
    } else { // no valid path found
        // 之前找到路径，但是到这一步断了（比如终点部分障碍物信息发生了变化，或者碰到了墙壁？得返回？）
        if (is_global_path_init_ && path_momentum_counter_ < gp_params_.momentum_thred) { // momentum go forward
            ...
            // 如果autoswith开着，而且之前认为不需要尝试的话，这时候就需要切换到尝试状态
            if (gp_params_.is_autoswitch && is_free_nav_goal_) { // autoswitch to attemptable navigation
                ...
                if (is_global_path_init_) {
                    ...
                    _nav_node_ptr = goal_ptr;   //直接把下一步设置为了终点？
                }
                is_free_nav_goal_ = false;  //切换到尝试
                return true;
            }
            // 如果autoswith关闭，或者autoswith开启，且开启了尝试，依然走到这一步，那说明找不到路径。
...
NavNodePtr GraphPlanner::NextNavWaypointFromPath(const NodePtrStack& global_path, const NavNodePtr goal_ptr) {
    ...
    nav_point_ptr = global_path[nav_idx];   //取出global_path的第二个nav_node（第一个是odom）
    float dist = (nav_point_ptr->position - odom_node_ptr_->position).norm();
    while (dist < gp_params_.converge_dist) {   //判断机器人走的距离够不够大，只有认为一步距离超过0.5才是有效的距离
        nav_idx ++;
...
void GraphPlanner::UpdateGoal(const Point3D& goal) {
    ...
    float min_dist = FARUtil::kNearDist;    //0.8
    for (const auto& node_ptr : current_graph_) {
        node_ptr->is_block_to_goal = false;
        //一般来说，第二个nav_node就是自己的odom，只不过他的navpoint为true
        if (node_ptr->is_navpoint) { 
            // check if goal is near internav node
            // 计算（is_navpoint）到 goal的距离，如果有一个is_navpoint离goal很近，那就用这个去导航
    ...
    //如果is_navpoint都没有一个接近goal，那么就用goal的坐标创建一个nav_node
    ...
    or_dist = this->EulerCost(odom_node_ptr_, goal_node_ptr_);
    std::cout<<or_dist<<std::endl;
...
void GraphPlanner::ReEvaluateGoalPosition(const NavNodePtr& goal_ptr, const bool& is_adjust_height)
{
    ...
    const bool is_origin_free = ContourGraph::IsPoint3DConnectFreePolygon(ori_pos_height, odom_node_ptr_->position) ? true : false; //判断odom和goal的连线有没有穿过polygon
    if (is_origin_free) {
        goal_ptr->position = ori_pos_height;
    } else { // reproject to nearby free space  在free_terrain_grid里找一个点去替代goal
        ...
        // 如果在free_terrain_grid找到了该点，且在free_terrain_grid的范围内
                                         ...
                                         const PointCloudPtr& freeCloudIn) 
{
    ...
    this->ResetFreeTerrainGridOrigin(origin_center);        //把goal当做origin点，每其他的每一个point转sub都是通过该origin来转变，从而达到缩放的一种一致性
    free_terrain_grid_->ReInitGrid(INIT_BIT);
    // evaluate goal freespace status
    is_goal_in_freespace_ = false;
    Eigen::Vector3i temp_sub;
    temp_sub.x() = FARUtil::kINF,temp_sub.y() = FARUtil::kINF,temp_sub.z() = FARUtil::kINF;
    if (!freeCloudIn->empty() || !obsCloudIn->empty()) { // process terrain cloud
        ...
        for (const auto& point : obsCloudIn->points) { // Set obstacle cloud in free terrain grid
            Eigen::Vector3i c_sub = free_terrain_grid_->Pos2Sub(point.x, point.y, grid_center_.z);  //把point转换到free_terrain_grid_下的sub
            if(c_sub == temp_sub){
                continue;
            }
            temp_sub = c_sub;
            for (int i = -C_IF; i <= C_IF; i++) {       //对该sub进行一个free_terrain_grid_扩充
                for (int j = -C_IF; j <= C_IF; j++) {
                    Eigen::Vector3i sub = c_sub;
                    sub.x() += i, sub.y() += j, sub.z() = 0;
                    if (free_terrain_grid_->InRange(sub)) {             //TODO 搞一个set存放sub 如果set里找到了sub 就不判断了
                        const int ind = free_terrain_grid_->Sub2Ind(sub);
                        free_terrain_grid_->GetCell(ind) = free_terrain_grid_->GetCell(ind) | OBS_BIT;      //这一步其实很多都不必要的
                    }
                }
            }
        }
    ...
    if (!is_goal_in_freespace_) { // check for freespace status if no terrain clouds around     当我们的目标点还没有被探索过的时候，这个肯定是没有terrain clouds的
        float min_dist = FARUtil::kINF;
        for (const auto& node_ptr : current_graph_) {
            if (!node_ptr->is_navpoint) continue;       //我们只寻找traj里的node
            const float cur_dist = (node_ptr->position - center).norm_flat();           //计算x-y平面的距离
